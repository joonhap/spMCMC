## test the spNUTS2 algorithm on the multivariate normal distribution
source('mvnormModel.R')
source('spNUTS2.R')

args = commandArgs(trailingOnly=TRUE)
#args = c(.01, 1, 1, .65, 1, 1) # eps, ran.eps, ran.cos, astar, adaptive.massInv, repNo
...
algo <- 'spNUTS2'; adaptive.eps <- TRUE; adaptive.massInv <- as.numeric(args[5])
...
runtime <- system.time( {
    N <- 20; jmax <- 13 # no more than 2^jmax jumps will be made from the kernel
    result <- spNUTS2(x0, M, N, eps, l, jmax, ran.eps, ran.cos, adaptive.eps=adaptive.eps, adaptive.massInv=adaptive.massInv, astar=astar)
###    result <- spNUTS2(x0, M, N, eps, l, jmax, ran.eps, ran.cos, massInv=diag(Sigma))
}
)
...
save(eps, algo, repNo, runtime, moveprob, log2len, sym, eps.trace, accep.prob.trace, ESS.coda, minESS.coda, minESS.coda.perSec, avgESS.coda, avgESS.coda.perSec, ESS.mcmcse, minESS.mcmcse, minESS.mcmcse.perSec, avgESS.mcmcse, avgESS.mcmcse.perSec, ESS.mcmcse.tukey, minESS.mcmcse.tukey, minESS.mcmcse.tukey.perSec, avgESS.mcmcse.tukey, avgESS.mcmcse.tukey.perSec, file=paste('data/2020Jan/mvnorm_',algo,'_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_astar_',astar,'_repNo_',repNo,'.RData',sep=''))
