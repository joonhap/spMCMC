...
#args <- c(.002, 1, 1, 5, 0.65, 1) # eps, ran.eps, ran.cos, N, astar, repNo

M <- 20200
burnout <- 200
Ma <- M-burnout
eps <- as.numeric(args[1]) # leapfrog trajectory unstable if >~ .0027
...
algo <- 'spNUTS1'; adaptive.eps <- TRUE; adaptive.massInv <- TRUE
N <- as.numeric(args[4])
...
save(eps, algo, repNo, runtime, moveprob, nfail, log2len, sym, eps.trace, accep.prob.trace, ESS.coda, minESS.coda, minESS.coda.perSec, avgESS.coda, avgESS.coda.perSec, file=paste('data/logit_',algo,'_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_N_',N,'_astar_',astar,'_repNo_',repNo,'.RData',sep=''))
