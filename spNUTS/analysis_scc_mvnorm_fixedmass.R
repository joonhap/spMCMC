mvnorm.spNUTS1.result <- function(eps, N, astar, repNo, M, burnout, adaptive.massInv) {
    filename <- paste('data/mvnorm_','spNUTS1','_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_N_',N,'_astar_',astar,'_repNo_',repNo,'.RData',sep='')
    load(filename)
...
mvnorm.spNUTS2.result <- function(eps, astar, repNo, M, burnout, adaptive.massInv) {
    filename <- paste('data/mvnorm_','spNUTS2','_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_astar_',astar,'_repNo_',repNo,'.RData',sep='')
    load(filename)
...
mvnorm.NUTS.result <- function(eps, astar, repNo, M, burnout, adaptive.massInv) {
    filename <- paste('data/mvnorm_','NUTS','_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_astar_',astar,'_repNo_',repNo,'.RData',sep='')
    load(filename)
...
mvnorm.HMC.result <- function(eps, astar, repNo, M, burnout, adaptive.massInv) {
    filename <- paste('data/mvnorm_','HMC','_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_astar_',astar,'_repNo_',repNo,'.RData',sep='')
    load(filename)
    return(c(as.numeric(runtime)[1], moveprob, minESS.coda, minESS.coda.perSec, avgESS.coda, avgESS.coda.perSec, eps.trace[length(eps.trace)]))
}
mvnorm.HMC.results <- function(eps, astar, repNoRange, M, burnout, adaptive.massInv) {
    ...
    colnames(output) <- c('runtime', 'moveprob', 'minESS', 'minESSps', 'avgESS', 'avgESSps', 'final_eps')
    output
}
mvnorm.spHMC.result <- function(eps, astar, repNo, M, burnout, adaptive.massInv) {
    filename <- paste('data/mvnorm_','spHMC','_eps_',signif(eps,digits=2),ifelse(adaptive.massInv,"","_fixedmass"),'_l_',50,'_astar_',astar,'_repNo_',repNo,'.RData',sep='')
    load(filename)
    return(c(as.numeric(runtime)[1], moveprob, minESS.coda, minESS.coda.perSec, avgESS.coda, avgESS.coda.perSec, eps.trace[length(eps.trace)]))
}
...
eps <- 0.01; repNoRange <- 1:10; M <- 20200; burnout <- 200; adaptive.massInv <- FALSE

mvnorm_HMC_.95 <- mvnorm.HMC.results(eps, .95, repNoRange, M, burnout, adaptive.massInv)
mvnorm_HMC_.85 <- mvnorm.HMC.results(eps, .85, repNoRange, M, burnout, adaptive.massInv)
mvnorm_HMC_.75 <- mvnorm.HMC.results(eps, .75, repNoRange, M, burnout, adaptive.massInv)
mvnorm_HMC_.65 <- mvnorm.HMC.results(eps, .65, repNoRange, M, burnout, adaptive.massInv)
mvnorm_HMC_.55 <- mvnorm.HMC.results(eps, .55, repNoRange, M, burnout, adaptive.massInv)
mvnorm_HMC_.45 <- mvnorm.HMC.results(eps, .45, repNoRange, M, burnout, adaptive.massInv)
##mvnorm_HMC_.35 <- mvnorm.HMC.results(eps, .35, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.95 <- mvnorm.spHMC.results(eps, .95, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.85 <- mvnorm.spHMC.results(eps, .85, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.75 <- mvnorm.spHMC.results(eps, .75, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.65 <- mvnorm.spHMC.results(eps, .65, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.55 <- mvnorm.spHMC.results(eps, .55, repNoRange, M, burnout, adaptive.massInv)
mvnorm_spHMC_.45 <- mvnorm.spHMC.results(eps, .45, repNoRange, M, burnout, adaptive.massInv)
##mvnorm_spHMC_.35 <- mvnorm.spHMC.results(eps, .35, repNoRange, M, burnout, adaptive.massInv)
...
avgESSps <- c(
mvnorm_HMC_.95[,"avgESSps"],
mvnorm_HMC_.85[,"avgESSps"],
mvnorm_HMC_.75[,"avgESSps"],
mvnorm_HMC_.65[,"avgESSps"],
mvnorm_HMC_.55[,"avgESSps"],
mvnorm_HMC_.45[,"avgESSps"],
##mvnorm_HMC_.35[,"avgESSps"],
mvnorm_spHMC_.95[,"avgESSps"],
mvnorm_spHMC_.85[,"avgESSps"],
mvnorm_spHMC_.75[,"avgESSps"],
mvnorm_spHMC_.65[,"avgESSps"],
mvnorm_spHMC_.55[,"avgESSps"],
mvnorm_spHMC_.45[,"avgESSps"],
##mvnorm_spHMC_.35[,"avgESSps"],
...
runtime <- c(
mvnorm_HMC_.95[,"runtime"],
mvnorm_HMC_.85[,"runtime"],
mvnorm_HMC_.75[,"runtime"],
mvnorm_HMC_.65[,"runtime"],
mvnorm_HMC_.55[,"runtime"],
mvnorm_HMC_.45[,"runtime"],
##mvnorm_HMC_.35[,"runtime"],
mvnorm_spHMC_.95[,"runtime"],
mvnorm_spHMC_.85[,"runtime"],
mvnorm_spHMC_.75[,"runtime"],
mvnorm_spHMC_.65[,"runtime"],
mvnorm_spHMC_.55[,"runtime"],
mvnorm_spHMC_.45[,"runtime"],
##mvnorm_spHMC_.35[,"runtime"],
...
if(gen_pdf) { pdf(file='figures/mvnormModel/minESSperSec_astar_algo_fixedmass.pdf', width=10, height=3) }
par(mar=c(4,4.5,.3,.3))
...
gen_pdf <- TRUE; cex <- 1.3
if(gen_pdf) { pdf(file='figures/mvnormModel/avgESSperSec_astar_algo_fixedmass.pdf', width=10, height=3) }
par(mar=c(4,4.5,.3,.3))
...
if(gen_pdf) { pdf(file='figures/mvnormModel/ESSperSec_astar_algo_fixedmass.pdf', width=10, height=6.5) }
par(mar=c(4,4.5,0.3,.3)); par(mfrow=c(3,1), oma=c(2.8,0,2,0))
## minimum ESS per second
plot(rep(1:(length(algorithms)*length(astarValues)),each=length(repNoRange)),minESSps, pch='.', xaxt='n',yaxt='n', xlab='',ylab='', xaxs="i", cex=cexpt, xlim=c(1-.5,(length(algorithms)*length(astarValues))+.5), ylim=c(min(minESSps[-c((2*length(astarValues)-1)*length(repNoRange)+repNoRange,(3*length(astarValues)-1)*length(repNoRange)+repNoRange)]),max(minESSps)), log='y') # exclude the NUTS minESSps for astar=0.45 (very small) to get a better range of y-axis
